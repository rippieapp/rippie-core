/**
 * iTunes Search API coverage probe.
 *
 * The Apple provider (`src/providers/appleMusic.ts`) relies entirely on the official, public
 * `itunes.apple.com/search` and `/lookup` endpoints. This script checks whether Search alone can
 * even find a given track, before any fuzzy matching or scoring is applied on top, since Search's
 * coverage has real gaps that Lookup does not share (see the explicit-content note in
...
if (songMatches.length === 0) {
	...
	console.log(
		`  Found via song search: ${songMatches.length} result(s), ${ids.length} unique id(s):`,
	)
	for (const id of ids) {
		console.log(
			`    id ${id}: ${songMatches.find((t) => t.trackId === id)?.trackViewUrl ?? '?'}`,
		)
	}
}

console.log(
	`Album sweep: ${allAlbumTracks.length} raw tracklist entries across all albums checked.`,
)
if (albumMatches.length === 0) {
	...
	console.log(
		`  Found inside an album tracklist: ${albumMatches.length} result(s), ${ids.length} unique id(s):`,
	)
	for (const id of ids) {
		console.log(
			`    id ${id}: ${albumMatches.find((t) => t.trackId === id)?.trackViewUrl ?? '?'}`,
		)
	}
}
