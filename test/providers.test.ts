...
describe('apple music provider', () => {
	...
	test('resolves a /song/ link the same way as an /album/ link', async () => {
		// Apple's "copy link" button on a search result now returns /song/ links instead of
		// /album/?i= ones. The id sits directly at the end of the path with no ?i= param at all.
		const http = mockFetch({
			'itunes.apple.com/lookup': {
				results: [
					{
		...
		expect(apple.matches('https://music.apple.com/us/song/unico/1864036284')).toBe(true)

		const track = await apple.fetchTrack('https://music.apple.com/us/song/unico/1864036284')
		expect(track?.isrc).toBe('US6R22582396')

		const [lookup] = http.matching('itunes.apple.com/lookup')
		expect(lookup?.url).toContain('id=1864036284')
	})
})
...
describe('extractId', () => {
	test('every built-in provider extracts a stable per-track id', async () => {
		...
		expect(await apple.extractId?.('https://music.apple.com/us/song/unico/1864036284')).toBe(
			'1864036284',
		)
